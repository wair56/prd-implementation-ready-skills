...
| A feature takes data from an external system, other module, upstream object, imported file, sync result, configuration, source detail list, or exposes an API/interface/wrapper API with upstream/cache/error/timeout behavior | `references/data-flow.md`, `references/page-ui.md`, `references/business-consistency.md`; add `references/api-interface.md` and `references/non-functional.md` for API contracts |
