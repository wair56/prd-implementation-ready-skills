...
Billing amount composition must explain source lineage. For monthly charter, do not write only "cost basis + service fee rate + service fee + adjustment lines": define where the cost basis comes from, usually the confirmed/locked cost bill or another named source; which cost inclusion states, cost types, period, vehicle/customer/contract scope, amortization lines, and locked period are included; and how users drill down to the cost bill and source cost details. If the vehicle/customer scope comes from a dedicated vehicle or vehicle-customer binding, define the binding record, relationship source, effective period, anchor time for the billing month, snapshot timing, rebuild policy, and correction behavior instead of writing only real-time fetch. For transport-volume billing, define the waybill/order amount source, eligible source detail set, occupation/snapshot timing, adjustment line source, and backtrace to original waybills/orders.
